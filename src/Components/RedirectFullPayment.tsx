...
const RedirectFullPayment: React.FC = () => {
  ...
  useEffect(() => {
    if (email) fetchInvoiceByEmail(email);
  }, [email]);
  ...
  const fetchInvoiceByEmail = async (email: string) => {
    ...
    try {
      ...
      if (invoiceData?.length) {
        ...
        navigate(`/create-invoice/full-payment/${invoice._id}`);
      } else {
        setError("No invoice found for this email.");
      }
    } catch (error: any) {
      console.error("Error fetching invoice:", error);
      setError("Network or server error while fetching invoice.");
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleProceedToPayment = async () => {
    if (!invoicedetails?.email || !invoicedetails?._id) {
    ...
    try {
      ...
      setError("Error during payment initialization.");
    }
  };
  ...
  return (
    <div className="createInvoice">
      ...
      {/* <div className="bye">{payOp}</div>
      <div className="bye">{setPayOp}</div> */}
    </div>
  );
};
